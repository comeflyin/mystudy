...
// 组件可以使用的数据状态 私有响应式数据 ref 由父组件提供的 props
...
const allCheck = computed(() => todos.value.every(todo => todo.done));  // computed todos every
const filterTodos = computed(() => filters[visibility.value](todos.value)) // computed todos + visibility
const remaining = computed(() => todos.value.filter(todo => !todo.done).length) // computed todos filterTodos
